...
describe RemoveOldStories do
  describe ".remove!" do
    ...
    it "should pass along the number of days to the story repository query" do
      expect(RemoveOldStories).to receive(:pruned_feeds).and_return([])
    ...
    it "should request deletion of all old stories" do
      expect(RemoveOldStories).to receive(:pruned_feeds).and_return([])
      expect(StoryRepository).to receive(:unstarred_read_stories_older_than).and_return(stories_mock)
    ...
    it "should fetch affected feeds by id" do
      expect(RemoveOldStories).to receive(:old_stories) do
        stories = [double("story", feed_id: 3), double("story", feed_id: 5)]
        expect(stories).to receive(:delete_all)
        stories
      end
    ...
    it "should update last_fetched on affected feeds" do
      ...
      expect(RemoveOldStories).to receive(:pruned_feeds) { feeds }
      expect(RemoveOldStories).to receive(:old_stories) { stories_mock }
